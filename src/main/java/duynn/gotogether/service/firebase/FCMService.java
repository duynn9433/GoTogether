...
import duynn.gotogether.dto.firebase.PnsRequest;
...
public class FCMService {
    ...
    private Message getPreconfiguredMessageToToken(PushNotificationRequest request) {
        if (request.getData() != null){
            return getPreconfiguredMessageWithData(request.getData(),request);
        }
    ...
    public void sendMessageToTopic(PushNotificationRequest request) throws FirebaseMessagingException {
        Message message = getPreconfiguredMessageWithoutData(request);
        String response = null;

        response = FirebaseMessaging.getInstance().send(message);

        logger.info("Sent message to topic: " + response);
    }
}
