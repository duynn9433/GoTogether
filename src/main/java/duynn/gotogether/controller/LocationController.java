...
import duynn.gotogether.dto.request.NewClientUpdateLocationRequest;
...
public class LocationController {
    ...
    @Autowired
    public LocationController(TripServiceImpl tripService, LocationServiceImpl locationService, ModelMapper modelMapper) {
        this.tripService = tripService;
        this.locationService = locationService;
        this.modelMapper = modelMapper;
    }

    @PostMapping("/new-update-driver-location")
    public ResponseEntity<?> newUpdateDriverLocation(@RequestBody NewClientUpdateLocationRequest request) {
        try{
...
//            Integer res = locationService.updateLocationByClientId(request.getClientId(), driverLocation);
            Location ret = locationService.newUpdateLocation(driverLocation);
            //get passenger locations
            List<Location> passengerLocations = locationService.getListLocationWithId(request.getClientLocationIDs());
    ...
    @PostMapping("/new-update-passenger-location")
    @Transactional
    public ResponseEntity<?> newUpdatePassengerLocation(@RequestBody NewClientUpdateLocationRequest request) {
        try{
            //update passenger location
            Location passengerLocation = modelMapper.map(request.getLocation(), Location.class);
            Location res = locationService.newUpdateLocation(passengerLocation);
            //get driver location
            Location driverLocation = locationService.newGetLocation(request.getDriverId());
